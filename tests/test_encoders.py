...
def test_bilingual_longsyllable():
    assert (
        LongSyllable().decode(LongSyllable().encode(bilingual_sent)) == bilingual_sent
    )
...
def test_comma():
    assert (
        Syllable().decode(Syllable().encode("39 tiriti o pipitea,"))
        == "39 tiriti o pipitea,"
    )
...
def test_quotation_mark():
    assert (
        Syllable().decode(Syllable().encode("‘kua whānau. aue!’"))
        == "‘kua whānau. aue!’"
    )
